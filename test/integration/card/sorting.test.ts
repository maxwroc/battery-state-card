...
describe("Entities correctly sorted", () => {
    const stateValueTests = [
        ["state", ["50", "90", "40"], "40 %, 50 %, 90 %"],
        ["state", ["40.5", "40.9", "40", "40.4"], "40 %, 40,4 %, 40,5 %, 40,9 %"],
    ];

    stateValueTests.forEach(([sort, entityStates, expectedOrder]) => {
        it(`when various state values appear - ${(entityStates as string[]).join(',')}`, async () => {
            ...
            const cardElem = hass.addCard("battery-state-card", {
                title: "Header",
                entities,
                sort: sort as ISimplifiedArray<ISortOption>
            });
    ...
    it("when HA state formatting returns various result formats", async () => {
        const formattedStates = ["50%", "90%", "40%"];
        const expectedOrder = "40 %, 50 %, 90 %";
        ...
        const cardElem = hass.addCard("battery-state-card", {
            title: "Header",
            entities,
            sort: "state" as ISimplifiedArray<ISortOption>
        });
    ...
    stateMapTests.forEach(([sort, formattedStates, expectedOrder]) => {
        it(`when state_map is used with display value - ${(formattedStates as string[]).length} states`, async () => {
            const hass = new HomeAssistantMock<BatteryStateCard>();
            const entities = (formattedStates as string[]).map((state, i) => {
                const batt = hass.addEntity(`Batt ${i + 1}`, state);
                return batt.entity_id;
            });
